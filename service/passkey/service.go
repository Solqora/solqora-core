	...
	if originsStr != "" {
		...
		for _, origin := range originList {
			...
			if !settings.AllowInsecureOrigin && strings.HasPrefix(strings.ToLower(trimmed), "http://") {
				return nil, fmt.Errorf("Passkey  Origin: %s", trimmed)
			}
...
autoDetect:
	...
	if scheme == "http" && !settings.AllowInsecureOrigin && r.Host != "localhost" && r.Host != "127.0.0.1" && !strings.HasPrefix(r.Host, "127.0.0.1:") && !strings.HasPrefix(r.Host, "localhost:") {
		return nil, fmt.Errorf("Passkey  HTTPS: %s://%s Passkey  Origin  HTTPS", scheme, r.Host)
	}
	// Host
	host := r.Host
	...
	if host == "" {
		return nil, fmt.Errorf(" Passkey  Origin Passkey  Host: '%s', ServerAddress: '%s'", r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New("Passkey  Origin RPID")
	}
	parsed, err := url.Parse(origins[0])
	if err != nil {
		return "", fmt.Errorf(" Passkey Origin: %w", err)
	}
