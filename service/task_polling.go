...
func sweepTimedOutTasks(ctx context.Context) {
	...
	reason := fmt.Sprintf("%d", constant.TaskTimeoutMinutes)
	legacyReason := ""
	now := time.Now().Unix()
	timedOutCount := 0
...
func UpdateSunoTasks(ctx context.Context, taskChannelM map[int][]string, taskM map[string]*model.Task) error {
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf(" #%d : %s", channelId, err.Error()))
		}
	}
	return nil
}
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	if !responseItems.IsSuccess() {
		common.SysLog(fmt.Sprintf(" #%d : %d, : %s", channelId, len(taskIds), string(responseBody)))
		return err
	}
	...
	for _, responseItem := range responseItems.Data {
		...
		if responseItem.FailReason != "" || task.Status == model.TaskStatusFailure {
			logger.LogInfo(ctx, task.TaskID+" "+task.FailReason)
			task.Progress = "100%"
...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	// try parse as Solqora response format
...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	...
	if bc := task.PrivateData.BillingContext; bc != nil && bc.PerCallBilling {
		logger.LogInfo(ctx, fmt.Sprintf(" %s ", task.TaskID))
		return
	}
	// 1.  adaptor 
	if actualQuota := adaptor.AdjustBillingOnComplete(task, taskResult); actualQuota > 0 {
		RecalculateTaskQuota(ctx, task, actualQuota, "adaptor")
		return
	}
	// 2.  token 
