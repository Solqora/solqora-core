...
const (
	...
	channelAffinityCacheNamespace           = "solqora-core:channel_affinity:v1"
	channelAffinityUsageCacheStatsNamespace = "solqora-core:channel_affinity_usage_cache_stats:v1"
)
...
func ClearChannelAffinityCacheByRuleName(ruleName string) (int, error) {
	...
	if ruleName == "" {
		return 0, fmt.Errorf("rule_name ")
	}

	setting := operation_setting.GetChannelAffinitySetting()
	if setting == nil {
		return 0, fmt.Errorf("channel_affinity_setting ")
	}
	...
	if !matchedRule.IncludeRuleName {
		return 0, fmt.Errorf(" include_rule_name")
	}
