...
const CACHE_FIELDS_TIMED = [
  { field: 'cache_read_unit_cost', labelKey: '' },
  { field: 'cache_create_unit_cost', labelKey: '5' },
  { field: 'cache_create_1h_unit_cost', labelKey: '1' },
];

const CACHE_FIELDS_GENERIC = [
  { field: 'cache_read_unit_cost', labelKey: '' },
  { field: 'cache_create_unit_cost', labelKey: '' },
];
      ...
      <Collapsible isOpen={expanded}>
        ...
        >
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            >
              <Radio value={CACHE_MODE_GENERIC}>{t('')}</Radio>
              <Radio value={CACHE_MODE_TIMED}>{t(' (Claude)')}</Radio>
            </RadioGroup>
          </div>
          <div
            style={{
    ...
    <div>
      <Banner
        ...
        description={
          <div>
            <div>
              {t('')}: <code>p</code> ({t(' Token')}), <code>c</code> (
              {t(' Token')}), <code>len</code> ({t('')}),{' '}
              <code>cr</code> ({t('')}),{' '}
              <code>cc</code> ({t('')}),{' '}
              <code>cc1h</code> ({t('-1')})
            </div>
            <div>
              {t('')}: <code>tier(name, value)</code>,{' '}
...
function RuleConditionRow({ cond, onChange, onRemove, t }) {
  ...
  const sourceSelect = (
    <Select
      ...
      style={{ width: 110 }}
    >
      <Select.Option value={SOURCE_PARAM}>{t('')}</Select.Option>
      <Select.Option value={SOURCE_HEADER}>{t('')}</Select.Option>
      <Select.Option value={SOURCE_TIME}>{t('')}</Select.Option>
    </Select>
  );
  ...
  return (
    <div style={{
      ...
      <Input
        ...
        placeholder={normalized.source === SOURCE_HEADER ? t(' anthropic-beta') : t(' service_tier')}
      ...
      <Input
        ...
        placeholder={normalized.mode === MATCH_CONTAINS ? t('') : normalized.mode === MATCH_EXISTS ? '' : t('')}
        disabled={!showValue}
...
const LLM_PROMPT_TEMPLATE = ` AI API billing expression AI API 
...
p  c  token cr token  p  token  p/c 
...
- header(name) — 
- param(path) —  JSON gjson 
- has(source, substr) — 
- hour(tz)minute(tz)weekday(tz)month(tz)day(tz) — tz  "Asia/Shanghai"

### 

 $/1M tokens  p * 2.5  $2.50/1M tokens
...
1.  tier("", ) 
2. tier  "base""standard""long_context"
3.  len p <<=>>=
4. 1 ? tier(...) : (2 ? tier(...) : tier(...))
5.  $/1M tokens 
6. // token  p/c 
  ...
  return (
    <div>
      ...
      <Card
        ...
        style={{ marginBottom: 12, background: 'var(--semi-color-fill-0)' }}
      >
        <div className='font-medium mb-2'>{t('Token ')}</div>
        ...
        >
          {evalResult.error ? (
            ...
            <div>
              ...
              >
                {t('')}{evalResult.cost.toLocaleString()}
              </Text>
            </div>
          )}
        </div>
