...
const OP_TAG_MAP = {
  [OP_ADD]: { color: 'green', label: ' (+:)' },
  [OP_REMOVE]: { color: 'red', label: ' (-:)' },
  [OP_APPEND]: { color: 'blue', label: '' },
};
    ...
    >
      ...
      >
        <div className='flex items-center gap-2'>
          ...
          <Tag size='small' color='blue'>{items.length} {t('')}</Tag>
        </div>
...
export default function GroupSpecialUsableRules({
  ...
  const opOptions = useMemo(
    () => [
      { value: OP_ADD, label: t(' (+:)') },
      { value: OP_REMOVE, label: t(' (-:)') },
      { value: OP_APPEND, label: t('') },
    ],
    [t],
  );
