...
export const buildSummaryText = (model, t) => {
  ...
  if (model.billingMode === 'tiered_expr') {
    ...
    if (!expr) return `${t('')}${requestRuleSuffix}`;
    const tierCount = (expr.match(/tier\(/g) || []).length;
    if (tierCount === 0) {
      return `${t('')}${requestRuleSuffix}`;
    }
    return `${t('')} (${tierCount} ${t('')})${requestRuleSuffix}`;
  }

  if (model.billingMode === 'per-request' && hasValue(model.fixedPrice)) {
    return `${t('')} $${model.fixedPrice} / ${t('')}${requestRuleSuffix}`;
  }
  ...
  if (hasValue(model.inputPrice)) {
    ...
    const extraLabel =
      extraCount > 0 ? `${t('')} ${extraCount}` : '';
    return `${t('')} $${model.inputPrice}${extraLabel}${requestRuleSuffix}`;
  }
    ...
    const rows = [
      {
        key: 'ModelPrice',
        label: 'ModelPrice',
        value: hasValue(model.fixedPrice) ? model.fixedPrice : t(''),
      },
    ];
    return rows;
  }
  ...
  const rows = [
    ...
    },
    {
      ...
      value: model.completionRatioLocked
        ? `${model.lockedCompletionRatio || t('')} (${t('')})`
    ...
    },
    {
      ...
      label: 'CacheRatio',
      value:
        cachePrice !== null ? formatNumber(cachePrice / inputPrice) : t(''),
    },
    {
      key: 'CreateCacheRatio',
      label: 'CreateCacheRatio',
    ...
    },
    {
      ...
      label: 'ImageRatio',
      value:
        imagePrice !== null ? formatNumber(imagePrice / inputPrice) : t(''),
    },
    {
      key: 'AudioRatio',
      label: 'AudioRatio',
