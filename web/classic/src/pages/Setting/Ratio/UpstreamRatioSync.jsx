...
function ConflictConfirmModal({ t, visible, items, loading, onOk, onCancel }) {
  ...
  const columns = [
    { title: t(''), dataIndex: 'channel' },
    { title: t(''), dataIndex: 'model' },
    {
      title: t(''),
      dataIndex: 'current',
...
export default function UpstreamRatioSync(props) {
  ...
  function getSyncFieldLabel(ratioType) {
    const typeMap = {
      ...
      completion_ratio: t(''),
      cache_ratio: t(''),
      create_cache_ratio: t(''),
      image_ratio: t(''),
      audio_ratio: t(''),
      audio_completion_ratio: t(''),
      model_price: t(''),
      billing_mode: t(''),
      billing_expr: t(''),
    };
  ...
  const applySync = async () => {
    ...
    Object.entries(resolutions).forEach(([model, ratios]) => {
      ...
      if (localCat && newCat !== 'tiered' && localCat !== newCat) {
        const currentDesc =
          localCat === 'price'
            ? `${t('')} : ${currentRatios.ModelPrice[model]}`
            : `${t('')} : ${currentRatios.ModelRatio[model] ?? '-'}\n${t('')} : ${currentRatios.CompletionRatio[model] ?? '-'}`;

        let newDesc = '';
        if (newCat === 'price') {
          newDesc = `${t('')} : ${ratios['model_price']}`;
        } else {
          const newModelRatio = ratios['model_ratio'] ?? '-';
          const newCompRatio = ratios['completion_ratio'] ?? '-';
          newDesc = `${t('')} : ${newModelRatio}\n${t('')} : ${newCompRatio}`;
        }
      ...
      <div className='flex flex-col md:flex-row justify-between items-center gap-4 w-full'>
        <div className='flex flex-col md:flex-row gap-2 w-full md:w-auto order-2 md:order-1'>
          ...
          <div className='flex flex-col sm:flex-row gap-2 w-full md:w-auto mt-2'>
            ...
            >
              <Select.Option value='model_ratio'>{t('')}</Select.Option>
              <Select.Option value='completion_ratio'>
                {t('')}
              </Select.Option>
              <Select.Option value='cache_ratio'>{t('')}</Select.Option>
              <Select.Option value='create_cache_ratio'>
                {t('')}
              </Select.Option>
              <Select.Option value='image_ratio'>{t('')}</Select.Option>
              <Select.Option value='audio_ratio'>{t('')}</Select.Option>
              <Select.Option value='audio_completion_ratio'>
                {t('')}
              </Select.Option>
              <Select.Option value='model_price'>{t('')}</Select.Option>
