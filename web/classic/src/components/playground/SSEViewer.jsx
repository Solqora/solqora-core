        ...
        {item.parsed?.choices?.[0] && (
          <div className='flex flex-wrap gap-2 text-xs'>
            {item.parsed.choices[0].delta?.content && (
              <Badge
                count={`${t('')}: "${String(item.parsed.choices[0].delta.content).substring(0, 20)}..."`}
                type='primary'
              />
            )}
            ...
            {item.parsed.choices[0].finish_reason && (
              <Badge
                count={`${t('')}: ${item.parsed.choices[0].finish_reason}`}
                type='success'
              />
            )}
            {item.parsed.usage && (
              <Badge
                count={`${t('')}: ${item.parsed.usage.prompt_tokens || 0}/${item.parsed.usage.completion_tokens || 0}`}
                type='tertiary'
              />
            )}
          </div>
    ...
    <div className='h-full flex flex-col bg-gray-50 dark:bg-gray-900/50 rounded-lg'>
      ...
      <div className='flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0'>
        <div className='flex items-center gap-3'>
          ...
          <Typography.Text strong>{t('SSE')}</Typography.Text>
          <Badge count={stats.total} type='primary' />
          {stats.errors > 0 && (
            <Badge count={`${stats.errors} ${t('')}`} type='danger' />
          )}
        </div>
