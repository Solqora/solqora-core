...
const PersonalSetting = () => {
  ...
  const startPasskeyRegistration = async () => {
    ...
    if (!methods.has2FA) {
      try {
        ...
        showError(error.message || t('Passkey '));
      }
      return;
    }
  ...
  const registerPasskey = async () => {
    ...
    try {
      ...
      if (!success) {
        throw new Error(message || t(' Passkey '));
      }
      ...
      if (!finishRes.data.success) {
        throw new Error(
          finishRes.data.message || t('Passkey '),
        );
      }

      showSuccess(t('Passkey '));
      ...
      throw new Error(error?.message || t('Passkey '));
    } finally {
