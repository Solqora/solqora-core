...
const AccountManagement = ({
  ...
  const loadCustomOAuthBindings = async () => {
    try {
      ...
      showError(error.response?.data?.message || error.message || t(''));
    }
  };
  ...
  const handleUnbindCustomOAuth = async (providerId, providerName) => {
    Modal.confirm({
      title: t(''),
      content: t(' {{name}} ', { name: providerName }),
      okText: t(''),
      cancelText: t(''),
      onOk: async () => {
              ...
              <Space vertical className='w-full'>
                ...
                <Card className='!rounded-xl w-full'>
                  <div className='flex flex-col sm:flex-row items-start sm:justify-between gap-4'>
                    <div className='flex items-start w-full sm:w-auto'>
                      ...
                      <div>
                        ...
                        <div className='mt-2 text-xs text-gray-500 space-y-1'>
                          ...
                          {/*      : t('')}*/}
                          {/*    {t('')}*/}
                          {/*    {passkeyStatus?.backup_state ? t('') : t('')}*/}
                          {/*  </div>*/}
                          {/*)}*/}
                          {!passkeySupported && (
                    ...
                    >
                      {passkeyEnabled ? t(' Passkey') : t(' Passkey')}
                    </Button>
                  </div>
                </Card>
