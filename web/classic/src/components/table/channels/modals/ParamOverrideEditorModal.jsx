...
const OPERATION_MODE_OPTIONS = [
  ...
  { label: '', value: 'move' },
  { label: '', value: 'replace' },
  { label: '', value: 'regex_replace' },
  { label: '', value: 'trim_prefix' },
  { label: '', value: 'trim_suffix' },
  { label: '', value: 'ensure_prefix' },
  { label: '', value: 'ensure_suffix' },
  { label: '', value: 'trim_space' },
  { label: '', value: 'to_lower' },
  { label: '', value: 'to_upper' },
  { label: '', value: 'return_error' },
  { label: '', value: 'prune_objects' },
  { label: '', value: 'pass_headers' },
  { label: '', value: 'sync_fields' },
  { label: '', value: 'set_header' },
  { label: '', value: 'delete_header' },
  { label: '', value: 'copy_header' },
  { label: '', value: 'move_header' },
];
...
const getModeFromLabel = (mode) => {
  ...
  if (mode === 'regex_replace') return '';
  if (mode === 'copy_header' || mode === 'move_header') return '';
  return '';
};
...
const getModeToLabel = (mode) => {
  if (mode === 'replace' || mode === 'regex_replace') return '';
  if (mode === 'copy_header' || mode === 'move_header') return '';
  return '';
};
...
const getModeValueLabel = (mode) => {
  if (mode === 'set_header') return ' JSON ';
  if (mode === 'pass_headers') return ' JSON ';
  if (
    mode === 'trim_prefix' ||
    mode === 'trim_suffix' ||
    mode === 'ensure_prefix' ||
    ...
    fields: [
      {
        key: 'model',
        label: '',
        tip: ' openai/gpt-4o-mini',
      },
      { key: 'temperature', label: '', tip: '' },
      { key: 'max_tokens', label: ' Token', tip: '' },
      { key: 'messages.-1.content', label: '', tip: '' },
    ],
  },
  {
    title: '',
    fields: [
      { key: 'retry.is_retry', label: '', tip: 'true ' },
      { key: 'last_error.code', label: '', tip: '' },
      {
...
const validateOperations = (operations, t) => {
  for (let i = 0; i < operations.length; i++) {
      ...
      try {
        ...
        if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
          if (!String(parsed.message || '').trim()) {
            return t(' {{line}}  return_error  message ', { line });
          }
        }
      } catch (error) {
    ...
    if (mode === 'prune_objects') {
      const raw = String(op.value_text ?? '').trim();
      if (!raw) {
        return t(' {{line}}  prune_objects ', { line });
      }
      try {
        const parsed = JSON.parse(raw);
        if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
...
const ParamOverrideEditorModal = ({ visible, value, onSave, onCancel }) => {
  ...
  const copyBuiltinField = async (fieldKey) => {
    ...
    if (ok) {
      showSuccess(t('{{name}}', { name: fieldKey }));
    } else {
      showError(t(''));
    }
  };
  ...
  const fieldGuideActionLabel = useMemo(() => {
    if (fieldGuideTarget === 'from') return t('');
    if (fieldGuideTarget === 'to') return t('');
    return t('');
  }, [fieldGuideTarget, t]);
          ...
          <div className='flex items-start justify-between gap-3'>
            <Space wrap spacing={8}>
              ...
              <Select
                ...
                style={{ width: 260 }}
              />
              <Button onClick={fillTemplateFromLibrary}>{t('')}</Button>
        ...
        {editMode === 'visual' ? (
          <div style={{ width: '100%' }}>
            {visualMode === 'legacy' ? (
              ...
              >
                <Text className='mb-2 block'>{t('JSON ')}</Text>
                <TextArea
                  value={legacyValue}
              ...
              <div>
                <div className='flex items-center justify-between mb-3'>
                  <Space>
                    <Text>{t(' + ')}</Text>
                    <Tag color='cyan'>{`${t('')}: ${operationCount}`}</Tag>
                  </Space>
          ...
          <div style={{ width: '100%' }}>
            <Space style={{ marginBottom: 8 }} wrap>
              <Button onClick={formatJson}>{t('')}</Button>
              <Tag color='grey'>{t('')}</Tag>
            </Space>
            <TextArea
              value={jsonText}
      ...
      >
        <Space vertical spacing={12} style={{ width: '100%' }}>
          <div className='flex items-start justify-between gap-3'>
            ...
            <Tag color='blue'>{`${fieldGuideFieldCount} ${t('')}`}</Tag>
          </div>
              ...
              {filteredFieldGuideSections.map((section) => (
                ...
                >
                  <div className='flex items-center justify-between mb-1'>
                    ...
                    <Tag color='grey'>{`${section.fields.length} ${t('')}`}</Tag>
                  </div>
                  <div
                    style={{
                      display: 'flex',
