...
const formatDurationSeconds = (seconds, t) => {
  ...
  const secs = total % 60;
  if (hours > 0) return `${hours}${tt('')} ${minutes}${tt('')}`;
  if (minutes > 0) return `${minutes}${tt('')} ${secs}${tt('')}`;
  return `${secs}${tt('')}`;
};
...
const formatAccountTypeLabel = (value, t) => {
  ...
  switch (normalized) {
    ...
    case 'enterprise':
      return 'Enterprise';
    default:
      return getDisplayText(value) || tt('');
  }
};
...
const resolveUsageStatusTag = (t, rateLimit) => {
  ...
  if (!rateLimit || Object.keys(rateLimit).length === 0) {
    return <Tag color='grey'>{tt('')}</Tag>;
  }
  if (rateLimit?.allowed && !rateLimit?.limit_reached) {
    return <Tag color='green'>{tt('')}</Tag>;
  }
  return <Tag color='red'>{tt('')}</Tag>;
};
  ...
  return (
    <div className='rounded-lg bg-semi-color-bg-0 p-3 shadow-[0_1px_2px_rgba(0,0,0,0.04)]'>
      ...
      <div className='mt-3 grid grid-cols-1 gap-2 text-xs text-semi-color-text-2 sm:grid-cols-2'>
        <div className='min-w-0'>
          <div className='text-[11px]'>{tt('')}</div>
...
const CodexUsageView = ({ t, record, payload, onCopy, onRefresh }) => {
  ...
  const errorMessage =
    payload?.success === false
      ? getDisplayText(payload?.message) || tt('')
      : '';
    ...
    <div className='flex flex-col gap-4'>
      ...
      <div className='rounded-lg bg-semi-color-fill-0 p-4'>
        ...
        <div className='mt-4 grid grid-cols-1 gap-3 md:grid-cols-2'>
          <InfoField t={tt} label={tt('')} value={email} onCopy={onCopy} />
          <InfoField t={tt} label={tt('')} value={channelLabel} />
          <InfoField
            t={tt}
            label='User ID'
            value={userId}
      ...
      >
        <Collapse.Panel header={tt(' JSON')} itemKey='raw-json'>
    ...
    return (
      <>
        ...
        <div className='flex items-center justify-center py-10'>
          <Spin spinning={true} size='large' tip={tt('...')} />
        </div>
      </>
    );
  }
