...
const ExtendDurationModal = ({
  ...
  const fetchDeploymentDetails = async (deploymentId) => {
    ...
    try {
      ...
      const errorMessage = t('') + (message ? `: ${message}` : '');
  ...
  const calculatePrice = async (hours, details) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const message = response.data.message || '';
        setPriceEstimation(null);
        setPriceError(t('') + (message ? `: ${message}` : ''));
      }
    } catch (error) {
      if (costRequestIdRef.current !== requestId) {
        return;
  ...
  const currentRemainingTime = deployment?.time_remaining || '0';
  const newTotalTime = `${currentRemainingTime} + ${durationHours}${t('')}`;
          ...
          <div className='flex items-center justify-between'>
            ...
            <div className='text-right'>
              ...
              <Text size='small' type='secondary'>
                {t('')}: <Text strong>{currentRemainingTime}</Text>
              </Text>
            </div>
          </div>
        </Card>
            ...
            <div className='space-y-3'>
              ...
              <div className='bg-blue-50 p-3 rounded-lg'>
                <div className='flex items-start gap-2'>
                  ...
                  <div>
                    <Text size='small' type='secondary'>
                      {t('')}: <Text strong>{newTotalTime}</Text>
                    </Text>
                    <br />
                    <Text size='small' type='secondary'>
                      {t('')}
