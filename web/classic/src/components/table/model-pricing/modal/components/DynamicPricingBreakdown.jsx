...
const VAR_LABELS = { p: '', c: '' };
const OP_LABELS = { '<': '<', '<=': '≤', '>': '>', '>=': '≥' };
const TIME_FUNC_LABELS = { hour: '', minute: '', weekday: '', month: '', day: '' };
...
function describeCondition(cond, t) {
  ...
  const src = cond.source === 'header' ? t('') : t('');
  const path = cond.path || '';
  if (cond.mode === MATCH_EXISTS) return `${src} ${path} ${t('')}`;
  if (cond.mode === MATCH_CONTAINS) return `${src} ${path} ${t('')} "${cond.value}"`;
  ...
  const tierColumns = [
    {
      ...
      render: (text, record) => (
        <div>
          <Tag color='blue' size='small'>{text || t('')}</Tag>
          {record.condSummary && (
