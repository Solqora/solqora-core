...
function buildChannelAffinityTooltip(affinity, t) {
  ...
  const lines = [
    t(''),
    `${t('')}${affinity.rule_name || '-'}`,
    `${t('')}${affinity.selected_group || '-'}`,
    `${t('Key')}${keyText}`,
    ...(keyHint ? [`${t('Key ')}${keyHint}`] : []),
  ];
...
function getUsageLogGroupSummary(groupRatio, userGroupRatio, t) {
  ...
  return `${useUserGroupRatio ? t('') : t('')} ${formatRatio(ratio)}x`;
}
  ...
  if (
    ...
    return {
      segments: [
        ...
        { text: t(''), tone: 'primary' },
        {
          text: `${t('')}${renderQuota(feeQuota, 6)}`,
          tone: 'secondary',
        },
        text ? { text: `${t('')}${text}`, tone: 'secondary' } : null,
      ].filter(Boolean),
    };
  }
      ...
      render: (text, record, index) => {
        ...
        if (other?.admin_info) {
          ...
          if (
            ...
            content = t('') + `${adminInfo.use_channel.join('->')}`;
          }
        ...
        return isAdminUser &&
          ...
          <Space>
            <span style={{ position: 'relative', display: 'inline-block' }}>
              <Tooltip content={record.channel_name || t('')}>
                <span>
                  <Tag
    ...
    },
    {
      ...
      title: (
        <div className='flex items-center gap-1'>
          ...
          <Tooltip
            content={t(
              ' Anthropic /v1/messages  tokens  tokens',
            )}
          >
      ...
      render: (text, record, index) => {
        ...
        if (hasCacheRead && hasCacheWrite) {
          cacheText = `${t('')} ${formatTokenCount(cacheSummary.cacheReadTokens)} · ${t('')} ${formatTokenCount(cacheSummary.cacheWriteTokens)}`;
        } else if (hasCacheRead) {
          cacheText = `${t('')} ${formatTokenCount(cacheSummary.cacheReadTokens)}`;
        } else if (hasCacheWrite) {
          cacheText = `${t('')} ${formatTokenCount(cacheSummary.cacheWriteTokens)}`;
        }
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        if (isSubscription) {
          ...
          return (
            <Tooltip content={`${t('')}${renderQuota(text, 6)}`}>
