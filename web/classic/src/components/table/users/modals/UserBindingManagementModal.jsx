...
const UserBindingManagementModal = ({
  ...
  const loadBindingData = React.useCallback(async () => {
    ...
    try {
      ...
      if (userRes.data?.success) {
        ...
        showError(userRes.data?.message || t(''));
      }
    } catch (error) {
      showError(
  ...
  const handleUnbindBuiltInAccount = (bindingItem) => {
    ...
    Modal.confirm({
      title: t(''),
      content: t(' {{name}} ', { name: bindingItem.name }),
      okText: t(''),
      cancelText: t(''),
      onOk: async () => {
  ...
  const handleUnbindCustomOAuthAccount = (provider) => {
    ...
    Modal.confirm({
      title: t(''),
      content: t(' {{name}} ', { name: provider.name }),
      okText: t(''),
      cancelText: t(''),
      onOk: async () => {
      ...
      <Spin spinning={bindingLoading}>
        <div className='max-h-[68vh] overflow-y-auto pr-1 pb-2'>
          <div className='flex items-center justify-between mb-4 gap-3 flex-wrap'>
            ...
            <Text type='tertiary'>
              {t('')} {boundCount} / {allBindingItems.length}
            </Text>
          </div>
