...
import (
	...
	taskcommon "github.com/solqora/solqora-core/relay/channel/task/taskcommon"
		...
		if files, exists := mf.File["input_reference"]; exists && len(files) > 0 {
			...
			for _, fileHeader := range files {
				...
				if fileHeader.Size > MaxFileSize {
					return nil, fmt.Errorf(" %s  %d MB", fileHeader.Filename, MaxFileSize/(1024*1024))
				}
...
func issolqoraRelay(apiKey string) bool {
