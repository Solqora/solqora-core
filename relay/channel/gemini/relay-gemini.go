...
import (
	...
	"github.com/solqora/solqora-core/setting/reasoning"
...
func geminiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response, callback func(data string, geminiResponse *dto.GeminiChatResponse) bool) (*dto.Usage, *types.solqoraError) {
	var usage = &dto.Usage{}
	var imageCount int
...
func GeminiChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.solqoraError) {
	id := helper.GetResponseID(c)
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.solqoraError) {
	...
	if len(geminiResponse.Candidates) == 0 {
		...
		if geminiResponse.PromptFeedback != nil && geminiResponse.PromptFeedback.BlockReason != nil {
			common.SetContextKey(c, constant.ContextKeyAdminRejectReason, fmt.Sprintf("gemini_block_reason=%s", *geminiResponse.PromptFeedback.BlockReason))
			solqoraError = types.NewOpenAIError(
		...
		service.ResetStatusCode(solqoraError, c.GetString("status_code_mapping"))
...
func GeminiEmbeddingHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.solqoraError) {
...
func GeminiImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.solqoraError) {
	responseBody, readErr := io.ReadAll(resp.Body)
	if readErr != nil {
		return nil, types.NewOpenAIError(readErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}
...
func FetchGeminiModels(baseURL, apiKey, proxyURL string) ([]string, error) {
	...
	for page := 0; page < maxPages; page++ {
		...
		if response.StatusCode != http.StatusOK {
			...
			response.Body.Close()
			cancel()
			return nil, fmt.Errorf(" %d: %s", response.StatusCode, string(body))
		}
