...
func authHelper(c *gin.Context, minRole int) {
	...
	// /root  AdminAuth/RootAuth
	// 
	// handler  ContextKeyAuditLoggedfinishAdminAudit 
...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if len(allowIps) > 0 {
			...
			if ip == nil {
				abortWithOpenAiMessage(c, http.StatusForbidden, " IP ")
				return
			}
			if common.IsIpInCIDRList(ip, allowIps) == false {
				abortWithOpenAiMessage(c, http.StatusForbidden, " IP ", types.ErrorCodeAccessDenied)
				return
			}
		...
		if tokenGroup != "" {
			...
			if _, ok := service.GetUserUsableGroups(userGroup)[tokenGroup]; !ok {
				abortWithOpenAiMessage(c, http.StatusForbidden, fmt.Sprintf(" %s ", tokenGroup))
				return
			}
