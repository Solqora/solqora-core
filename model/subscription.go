...
import (
	...
	"github.com/solqora/solqora-core/pkg/cachex"
...
const (
	subscriptionPlanCacheNamespace     = "solqora-core:subscription_plan:v1"
	subscriptionPlanInfoCacheNamespace = "solqora-core:subscription_plan_info:v1"
)
...
func CompleteSubscriptionOrder(tradeNo string, providerPayload string, expectedPaymentProvider string, actualPaymentMethod string) error {
	...
	if logUserId > 0 {
		msg := fmt.Sprintf(": %s: %.2f: %s", logPlanTitle, logMoney, logPaymentMethod)
...
func AdminBindSubscription(userId int, planId int, sourceNote string) (string, error) {
	...
	if strings.TrimSpace(plan.UpgradeGroup) != "" {
		...
		return fmt.Sprintf(" %s", plan.UpgradeGroup), nil
	}
	return "", nil
}
...
func PurchaseSubscriptionWithBalance(userId int, planId int) error {
	...
	msg := fmt.Sprintf(": %s: %.2f: %d", logPlanTitle, logMoney, chargedQuota)
...
func AdminInvalidateUserSubscription(userSubscriptionId int) (string, error) {
	...
	if downgradeGroup != "" {
		return fmt.Sprintf(" %s", downgradeGroup), nil
	}
	return "", nil
}
