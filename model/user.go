...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	if float64(quota) < common.QuotaPerUnit {
		return fmt.Errorf("%s", logger.LogQuota(int(common.QuotaPerUnit)))
	}
...
func (user *User) Insert(inviterId int) error {
	...
	if err := DB.Where("username = ?", user.Username).First(&createdUser).Error; err == nil {
		...
		if defaultSidebarConfig != "" {
			...
			common.SysLog(fmt.Sprintf(" %s (: %d) ", createdUser.Username, createdUser.Role))
		}
	}

	if common.QuotaForNewUser > 0 {
		RecordLog(user.Id, LogTypeSystem, fmt.Sprintf(" %s", logger.LogQuota(common.QuotaForNewUser)))
	}
	if inviterId != 0 && operation_setting.IsPaymentComplianceConfirmed() {
		if common.QuotaForInvitee > 0 {
			_ = IncreaseUserQuota(user.Id, common.QuotaForInvitee, true)
			RecordLog(user.Id, LogTypeSystem, fmt.Sprintf(" %s", logger.LogQuota(common.QuotaForInvitee)))
		}
		if common.QuotaForInviter > 0 {
			//_ = IncreaseUserQuota(inviterId, common.QuotaForInviter)
			RecordLog(inviterId, LogTypeSystem, fmt.Sprintf(" %s", logger.LogQuota(common.QuotaForInviter)))
...
func (user *User) FillUserByTelegramId() error {
	if user.TelegramId == "" {
		return errors.New("Telegram id ")
	}
	err := DB.Where(User{TelegramId: user.TelegramId}).First(user).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return errors.New(" Telegram ")
	}
	return nil
}
