...
func SubscriptionRequestStripePay(c *gin.Context) {
	...
	if plan.StripePriceId == "" {
		common.ApiErrorMsg(c, " StripePriceId")
		return
	}
	if !strings.HasPrefix(setting.StripeApiSecret, "sk_") && !strings.HasPrefix(setting.StripeApiSecret, "rk_") {
		common.ApiErrorMsg(c, "Stripe ")
		return
	}
	if setting.StripeWebhookSecret == "" {
		common.ApiErrorMsg(c, "Stripe Webhook ")
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe  trade_no=%s plan_id=%d error=%q", referenceId, plan.Id, err.Error()))
