...
func SubscriptionRequestCreemPay(c *gin.Context) {
	...
	if plan.CreemProductId == "" {
		common.ApiErrorMsg(c, " CreemProductId")
		return
	}
	if setting.CreemWebhookSecret == "" && !setting.CreemTestMode {
		common.ApiErrorMsg(c, "Creem Webhook ")
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem  trade_no=%s product_id=%s error=%q", referenceId, product.ProductId, err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": ""})
		return
	}
