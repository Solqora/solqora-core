...
// 
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/solqora/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/solqora/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/solqora/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/solqora/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/solqora/models.json", base), fmt.Sprintf("%s/api/solqora/vendors.json", base)
}
...
func SyncUpstreamModels(c *gin.Context) {
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": ": " + fetchErr.Error(), "locale": req.Locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
...
func SyncUpstreamPreview(c *gin.Context) {
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": ": " + fetchErr.Error(), "locale": locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
