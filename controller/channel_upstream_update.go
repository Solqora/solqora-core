...
func fetchChannelUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	...
	if channel.Type == constant.ChannelTypeGemini {
		...
		if apiErr != nil {
			return nil, fmt.Errorf(": %w", apiErr)
		}
...
func buildUpstreamModelUpdateTaskNotificationContent(
	...
	if len(channelSummaries) > 0 {
		...
		builder.WriteString(fmt.Sprintf("\n\n %d/%d", displayCount, len(channelSummaries)))
		...
		if len(channelSummaries) > displayCount {
			builder.WriteString(fmt.Sprintf("\n-  %d ", len(channelSummaries)-displayCount))
		}
	}
	...
	if len(normalizedAddModelSamples) > 0 {
		...
		if len(normalizedAddModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" %d ", len(normalizedAddModelSamples)-displayCount))
		}
	}
	...
	if len(normalizedRemoveModelSamples) > 0 {
		...
		if len(normalizedRemoveModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" %d ", len(normalizedRemoveModelSamples)-displayCount))
		}
	}
	...
	if failedChannels > 0 {
		...
		if failedChannels > displayCount {
			builder.WriteString(fmt.Sprintf(" %d ", failedChannels-displayCount))
		}
	}
