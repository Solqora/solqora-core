...
import (
	...
	"github.com/solqora/solqora-core/pkg/billingexpr"
...
func testChannel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	...
	solqoraError := middleware.SetupContextForSelectedChannel(c, channel, testModel)
	if solqoraError != nil {
		return testResult{
			context:     c,
			localErr:    solqoraError,
			solqoraError: solqoraError,
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			solqoraError: types.NewError(err, types.ErrorCodeJsonMarshalFailed),
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			solqoraError: types.NewError(err, types.ErrorCodeChannelModelMappedError),
		}
	}
	...
	if info.RelayMode == relayconstant.RelayModeResponsesCompact &&
		...
		return testResult{
			...
			solqoraError: types.NewError(fmt.Errorf("unsupported api type: %d", apiType), types.ErrorCodeInvalidApiType),
		}
	}
	...
	if adaptor == nil {
		return testResult{
			...
			solqoraError: types.NewError(fmt.Errorf("invalid api type: %d, adaptor is nil", apiType), types.ErrorCodeInvalidApiType),
		}
	}
	...
	case relayconstant.RelayModeEmbeddings:
		...
		if embeddingReq, ok := request.(*dto.EmbeddingRequest); ok {
			...
			return testResult{
				...
				solqoraError: types.NewError(errors.New("invalid embedding request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	case relayconstant.RelayModeImagesGenerations:
		...
		if imageReq, ok := request.(*dto.ImageRequest); ok {
			...
			return testResult{
				...
				solqoraError: types.NewError(errors.New("invalid image request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	case relayconstant.RelayModeRerank:
		...
		if rerankReq, ok := request.(*dto.RerankRequest); ok {
			...
			return testResult{
				...
				solqoraError: types.NewError(errors.New("invalid rerank request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	case relayconstant.RelayModeResponses:
		...
		if responseReq, ok := request.(*dto.OpenAIResponsesRequest); ok {
			...
			return testResult{
				...
				solqoraError: types.NewError(errors.New("invalid response request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	case relayconstant.RelayModeResponsesCompact:
		...
		default:
			return testResult{
				...
				solqoraError: types.NewError(errors.New("invalid response compaction request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	default:
		// Chat/Completion 
		if generalReq, ok := request.(*dto.GeneralOpenAIRequest); ok {
			...
			return testResult{
				...
				solqoraError: types.NewError(errors.New("invalid general request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			solqoraError: types.NewError(err, types.ErrorCodeConvertRequestFailed),
		}
	}
		...
		if err != nil {
			if fixedErr, ok := relaycommon.AsParamOverrideReturnError(err); ok {
				return testResult{
					context:     c,
					localErr:    fixedErr,
					solqoraError: relaycommon.solqoraErrorFromParamOverride(fixedErr),
				}
			}
			return testResult{
				context:     c,
				localErr:    err,
				solqoraError: types.NewError(err, types.ErrorCodeChannelParamOverrideInvalid),
			}
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			solqoraError: types.NewOpenAIError(err, types.ErrorCodeDoRequestFailed, http.StatusInternalServerError),
		}
	}
	...
	if resp != nil {
		...
		if httpResp.StatusCode != http.StatusOK {
			...
			return testResult{
				context:     c,
				localErr:    err,
				solqoraError: types.NewOpenAIError(err, types.ErrorCodeBadResponse, http.StatusInternalServerError),
			}
		}
	}
	...
	if usageErr != nil {
		return testResult{
			context:     c,
			localErr:    usageErr,
			solqoraError: types.NewOpenAIError(usageErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError),
		}
	}
	result := w.Result()
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			solqoraError: types.NewOpenAIError(err, types.ErrorCodeReadResponseBodyFailed, http.StatusInternalServerError),
		}
	}
	if bodyErr := validateTestResponseBody(respBody, isStream); bodyErr != nil {
		return testResult{
			context:     c,
			localErr:    bodyErr,
			solqoraError: types.NewOpenAIError(bodyErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError),
		}
	}
...
func TestChannel(c *gin.Context) {
	...
	if result.localErr != nil {
		...
		if result.solqoraError != nil {
			resp["error_code"] = result.solqoraError.GetErrorCode()
		}
	...
	if result.solqoraError != nil {
		c.JSON(http.StatusOK, gin.H{
			"success":    false,
			"message":    result.solqoraError.Error(),
			"time":       consumedTime,
			"error_code": result.solqoraError.GetErrorCode(),
		})
		return
	}
	c.JSON(http.StatusOK, gin.H{
...
func performChannelTests(ctx context.Context, channels []*model.Channel, testUserID int, allowDisable bool, report func(processed, total int)) channelTestSummary {
	...
	for index, channel := range channels {
		...
		solqoraError := result.solqoraError
		// request error disables the channel
		if solqoraError != nil {
			shouldBanChannel = service.ShouldDisableChannel(result.solqoraError)
		}

		// 
		if common.AutomaticDisableChannelEnabled && !shouldBanChannel {
			if milliseconds > disableThreshold {
				err := fmt.Errorf(" %.2fs  %.2fs", float64(milliseconds)/1000.0, float64(disableThreshold)/1000.0)
				solqoraError = types.NewOpenAIError(err, types.ErrorCodeChannelResponseTimeExceeded, http.StatusRequestTimeout)
		...
		if allowDisable && isChannelEnabled && shouldBanChannel && channel.GetAutoBan() {
			processChannelError(result.context, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(result.context, constant.ContextKeyChannelKey), channel.GetAutoBan()), solqoraError)
			summary.Disabled++
		}

		// enable channel
		if result.localErr == nil && !isChannelEnabled && service.ShouldEnableChannel(solqoraError, channel.Status) {
