				...
				if modelName, ok := taskData["model"].(string); ok && modelName != "" {
					...
					if hasRatioSetting && modelRatio > 0 {
						...
						if group != "" {
							...
							if quotaDelta > 0 {
								// 
								logger.LogInfo(ctx, fmt.Sprintf(" %s %s%s%stokens%d",
									task.TaskID,
								...
								if err := model.DecreaseUserQuota(task.UserId, quotaDelta, false); err != nil {
									logger.LogError(ctx, fmt.Sprintf(": %s", err.Error()))
								} else {
									...
									// 
									logContent := fmt.Sprintf(" %.2f %.2ftokens %d %s %s %s",
								...
								// quotaDelta == 0, 
								logger.LogInfo(ctx, fmt.Sprintf(" %s %stokens%d",
