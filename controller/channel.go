...
import (
	...
	relaychannel "github.com/solqora/solqora-core/relay/channel"
...
func GetChannelKey(c *gin.Context) {
	...
	if err != nil {
		common.ApiError(c, fmt.Errorf("ID: %v", err))
		return
	}
...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	if err := channel.ValidateSettings(); err != nil {
		return fmt.Errorf("[channel setting] %s", err.Error())
	}
	...
	if channel.Type == constant.ChannelTypeVertexAi {
		...
		if err != nil {
			return fmt.Errorf("Json{\"default\": \"us-central1\", \"region2\": \"us-east1\"}")
		}
...
func getVertexArrayKeys(keys string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(" Vertex AI JsonArray[{key1}, {key2}...]: %w", err)
	}
	...
	for _, key := range keyArray {
		...
		default:
			...
			if err != nil {
				return nil, fmt.Errorf("Vertex AI key JSON : %w", err)
			}
			keyStr = string(bytes)
		}
		if keyStr != "" {
	...
	if len(cleanKeys) == 0 {
		return nil, fmt.Errorf(" Vertex AI  keys ")
	}
...
func FetchModels(c *gin.Context) {
	...
	if req.Type == constant.ChannelTypeOllama {
		...
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": fmt.Sprintf("Ollama: %s", err.Error()),
			})
			return
		}
	...
	if req.Type == constant.ChannelTypeGemini {
		...
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": fmt.Sprintf("Gemini: %s", err.Error()),
			})
			return
		}
...
//
//	suffix         - string appended to the original name (default "_")
...
func CopyChannel(c *gin.Context) {
	...
	suffix := c.DefaultQuery("suffix", "_")
	resetBalance := true
...
func ManageMultiKeys(c *gin.Context) {
	...
	case "enable_all_keys":
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf(" %d ", enabledCount),
		})
		return
	...
	case "disable_all_keys":
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf(" %d ", disabledCount),
		})
		return
	...
	case "delete_disabled_keys":
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf(" %d ", deletedCount),
			"data":    deletedCount,
		})
		return
